...
EnemyBullet::EnemyBullet(int16_t origin_x, uint8_t origin_y, uint16_t inplayer_x, uint8_t inplayer_y){
 //init x and y position local variables based on the position of the enemy
	...
	determineSlope(inplayer_x, inplayer_y);
...
void EnemyBullet::Draw(){
	...
	if(this->x > SCREENWIDTH || this->x < 0 || this->y > 60 || this->y < 5){ //if bullet goes out of bound we return and mark it as dead to be able to erase it form the linked list 
	...
	} else{ //meaning we are moving with some slope and change in y > change in x because abs(slope) >= 1
